public class Moomintroll extends Human {
    ...
    public void lookon(Decor.Bed w, IHuman n) {
            try {
                if (w.getEngaged() == false) {
                    throw new StoryException(this + " Нет продолжения сюжета.");
                }
    ...
    public void hear(IHuman a, APlace b, Music c, Music.Song d) {
       place = b;
        System.out.println("Муми-тролль услышал, как " +  place.getPlace() + a.getName() + " Играл на музыкальном инструменте " + c.getHarmonice() + " : " + d.getHey_animals());
    }
